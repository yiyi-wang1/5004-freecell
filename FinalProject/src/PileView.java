		...
		for(int i = 0; i < 4; i++) {
			if(foundations.get(i).isEmpty()) {//if the foundation is empty
				...
				try {
					...
					CardImage f1 = new CardImage (X_INTERVAL1+i* (X_INTERVAL1 + CARD_WIDTH), Y_INTERVAL1, ImageIO.read(new File(name)), CARD_WIDTH, CARD_HEIGHT);
					this.add(f1);
					f1.paintComponent(g);
				} catch (IOException e) {
	...
	public void paintFreecells(Graphics g, ArrayList<ArrayList<Card>> freecells) {
		...
		for(int i = 0; i < 4; i++) {
			if(freecells.get(i).isEmpty()) {
				...
				try {
					CardImage f1 = new CardImage (X_INTERVAL2+i* (X_INTERVAL1 + CARD_WIDTH),Y_INTERVAL1,ImageIO.read(new File(name)), CARD_WIDTH,CARD_HEIGHT);
					this.add(f1);
					f1.paintComponent(g);
				} catch (IOException e) {
			...
			if(cascades.get(i).isEmpty()) {//if the cascade is empty, do not need to draw the cards
				...
				for(int j = 0; j < cascades.get(i).size(); j++) {
					...
					try {
						...
						//use j to determine the y coorindate
						CardImage c1 = new CardImage(X_INTERVAL3+i* (X_INTERVAL1 + CARD_WIDTH), Y_INTERVAL2+j*Y_INTERVAL3, ImageIO.read(new File(name)), CARD_WIDTH, CARD_HEIGHT);
						this.add(c1);
						c1.paintComponent(g);
					} catch (IOException e) {
						System.out.println("Unable to read the file:" + name);
